#!/bin/bash
set -ex
__ScriptName="offline_update_daphne.sh"
  ...
  cat << EOT
    ...
    Example:
      1) Use long options to build app:
        $ sudo $__ScriptName  --release-version=daphne-0.0.2 --srv-ip=192.168.201.97 --app-level=standard --prod-id=whzxyy-001
EOT
}
