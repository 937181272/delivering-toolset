...
__ScriptName="offline_update_cypress.sh"
  ...
  cat << EOT
    ...
    Example:
      1) Use long options to build app:
        $ sudo $__ScriptName --release-user=test --release-password=test --release-version=0.0.2 --srv-ip=192.168.1.1 --app-level=standard --prod-id=whzxyy-001
EOT
}

WORKSPACE=/home/devops1/sk
APP_NAME=medicalbrain-cta-cypress
...
# parse options:  
#RET=`getopt -o hp:v:i: -a -l help,source-dir:,release-version:,srv-ip: -n 'ERROR' -- "$@"`
