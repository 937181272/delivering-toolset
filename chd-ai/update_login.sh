  ...
  cat << EOT
    ...
    Example:
      1) Use long options to build app:
        $ sudo $__ScriptName --release-password=test --release-version=0.0.2 --srv-ip=192.168.1.1 --prod-id=zdyfy-001
EOT
}

WORKSPACE=/home/devops1/sk
APP_NAME=shukun-demo-login
...
# parse options:  
RET=`getopt -o hp:v:i:j: -a -l help,release-user:,release-password:,release-version:,srv-ip: -n 'ERROR' -- "$@"`
