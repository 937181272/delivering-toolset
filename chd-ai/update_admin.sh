  ...
  cat << EOT
    ...
    Example:
      1) Use long options to build app:
        $ sudo $__ScriptName --release-user=test --release-password=test --release-version=0.0.2 --srv-ip=192.168.1.1 --prod-id=zdyfy-001
EOT
}
...
# parse options:  
RET=`getopt -o hp:v:i:j: -a -l help,release-password:,release-version:,srv-ip:,prod-id: -n 'ERROR' -- "$@"`
...
echo " > download $NEW_VERSION daphne"
